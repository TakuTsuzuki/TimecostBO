...
from utils import fit, gauss_hermite, gpmean, minimize, brute_grid, filter_threthold2d
from cost import costfunc1

@jit
def ei_per_cost(x,func_cost,bounds,GP_model,jitter=0.):
    if len(x.shape) == 1:
        x = np.array([x])
...
def policy_naive_ei(GP_model, func_cost, depth_c,threthold,bounds):
    # caluc naive policy function
    func2minimize = lambda x : -1*ei(x,bounds,GP_model)
    query = minimize(func2minimize, bounds)
    return query

def policy_rollout_eipercost(GP_model, func_cost, depth_c,threthold,bounds):
    # caluc policy function
    if depth_c > threthold:
